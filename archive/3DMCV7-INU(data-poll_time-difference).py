    ...
    try:
        ...
        while True:
            ...
            if data:  # Only print if we got valid data
                message = f"Acceleration: X={data['x']:.6f}, Y={data['y']:.6f}, Z={data['z']:.6f} m/s^2 Time of Week: {data['time_of_week']:.6f}, Week Number: {data['week_number']}, Difference: {data['time_of_week']%60 - float(datetime.now().strftime('%S.%f')):.6f} s"
                print(message)
                log.write(message + '\n')
